...
public:
	...
	{
		return GetRateFromDB(player, CHAR_SEL_CUSTOM_LOOT_RATE);
	}
	...
	{
		SaveRateToDB(player, rate, update, CHAR_UPD_CUSTOM_LOOT_RATE, CHAR_INS_CUSTOM_LOOT_RATE);
	}
};
...
public:
	...
	void OnDelete(uint64 guid)
	{
		...
		CustomRates::DeleteRateFromDB(guid, CHAR_DEL_CUSTOM_LOOT_RATE);
	}
	...
	void OnLogin(Player *player)
	{
		...
		if (sWorld->getBoolConfig(CONFIG_CUSTOM_LOOT_RATE))
		{
			...
			if (rate != -1)
			{
				...
				if (sWorld->getBoolConfig(CONFIG_CUSTOM_LOOT_RATE_SHOW_ON_LOGIN))
				{
					if (uRate)
...
public:
	...
	std::vector<ChatCommand> GetCommands() const override
	{
		...
		static std::vector<ChatCommand> lootCommandTable =
		{
			{ "set", SEC_PLAYER, false, &HandleSetLootRateCommand, "" },
			{ "get", SEC_PLAYER, false, &HandleGetLootRateCommand, "" },
			{ "active", SEC_ADMINISTRATOR, false, &HandleLootRateToggleCommand , "" }
		};

		static std::vector<ChatCommand> rateTypeCommandTable =
		{
			{ "xp", SEC_PLAYER, false, nullptr, "", xpCommandTable },
			{ "loot", SEC_PLAYER, false, nullptr, "", lootCommandTable }
		};
	...
	static bool HandleLootRateToggleCommand(ChatHandler *handler, char const *args)
	{
		// Attempt to retrieve the player who executed the command.
		Player *me = handler->GetSession() ? handler->GetSession()->GetPlayer() : NULL;
		if (!me)
		...
		if (!*args)
		{
			handler->PSendSysMessage("Usage: .rate loot active [0/1]");
			handler->SetSentErrorMessage(true);
			return false;
		}
		...
		// No need to update this.
		if (toggle == sWorld->getBoolConfig(CONFIG_CUSTOM_LOOT_RATE))
		{
			handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Custom rates for loot are already %s.", toggle ? "on" : "off");
			handler->SetSentErrorMessage(true);
			return false;
		}

		// Update bool config.
		handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Custom rates for loot are now %s.", toggle ? "on" : "off");
		sWorld->setBoolConfig(CONFIG_CUSTOM_LOOT_RATE, toggle);
		return true;
	}
	...
	static bool HandleGetLootRateCommand(ChatHandler *handler, const char *args)
	{
		// take a pointer to the player who uses the command
		Player *me = handler->GetSession() ? handler->GetSession()->GetPlayer() : NULL;
		if (!me)
		...
		if (!sWorld->getBoolConfig(CONFIG_CUSTOM_LOOT_RATE))
		{
			handler->SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Viewing your loot rate is disabled at this time.");
			return true;
		}
	...
	static bool HandleSetLootRateCommand(ChatHandler *handler, const char *args)
	{
		// take a pointer to the player who uses the command
		Player *me = handler->GetSession() ? handler->GetSession()->GetPlayer() : NULL;
		if (!me)
		...
		if (!sWorld->getBoolConfig(CONFIG_CUSTOM_LOOT_RATE))
		{
			handler->SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Changing your loot rate is disabled at this time.");
			return true;
		}
		...
		// first, check if I can use the command
		if (me->GetSession()->GetSecurity() < (int)sWorld->getIntConfig(CONFIG_CUSTOM_LOOT_RATE_SECURITY))
		{
			handler->SendSysMessage(LANG_YOURS_SECURITY_IS_LOW);
            handler->SetSentErrorMessage(true);
            return false;
		...
		// extract value
		int rate = atoi((char *)args);
		int maxRate = sWorld->getIntConfig(CONFIG_CUSTOM_LOOT_RATE_MAX);
		if (rate <= 0 || rate > maxRate)
		{
			handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Invalid rate specified, must be in interval [1,%i].", maxRate);
			handler->SetSentErrorMessage(true);
			return false;
		}
