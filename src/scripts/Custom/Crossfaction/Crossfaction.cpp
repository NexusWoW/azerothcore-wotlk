...
void CrossFaction::DoForgetPlayersInBG(Battleground* pBattleGround, Player* player)
{
    ...
    {
        sLog->outDebug(LOG_FILTER_MODULES, "player %s - Reset Cache", player->GetName().c_str());
...
void CrossFaction::SetFakeRaceAndMorph(Player* player)
{
    if (player->getClass() == CLASS_DRUID)
    {
        if (player->GetTeamId(true) == TEAM_ALLIANCE) // ALLT
        {
            ...
            sLog->outDebug(LOG_FILTER_MODULES, "player %s is a tauren !", player->GetName().c_str());
        }
        else // HORDE
        {
            ...
            sLog->outDebug(LOG_FILTER_MODULES, "player %s is a nightelf !", player->GetName().c_str());
        }         
    }
    else
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_MODULES, "player %s is a human !", player->GetName().c_str());
        ...
        {
            sLog->outDebug(LOG_FILTER_MODULES, "player %s is a bloodelf now!", player->GetName().c_str());
...
void CrossFaction::UpdatePlayerTeam(Group* group, uint64 guid, bool reset /* = false */)
{
    ...
    if (player)
    {
        ...
        if (isMapEnabled(player->GetMapId()) && isZoneEnabled(player->GetZoneId()) && isAreaEnabled(player->GetAreaId()))
            ...
            sLog->outDebug(LOG_FILTER_MODULES, "Crossfaction: disabled for player %s", player->GetName().c_str());
        ...
        if (!reset && !disable)
        {
            ...
            if (player->InBattleground())
            {
                if (Battleground * bg = player->GetBattleground())
                {
                    ...
                    {
                        sLog->outDebug(LOG_FILTER_MODULES, "player %s switched faction!", player->GetName().c_str());
                        ...
                        sLog->outDebug(LOG_FILTER_MODULES, "Crossfaction: Battleground team id set for player %s", player->GetName().c_str());
                    }
                }
                return;
            }
            ...
            sLog->outDebug(LOG_FILTER_MODULES, "reset morph for player", player->GetGUIDLow());
        ...
        sLog->outDebug(LOG_FILTER_MODULES, "reset morph done for player %u", player->GetGUIDLow());

        sLog->outDebug(LOG_FILTER_MODULES, "Crossfaction: reset done for player %s", player->GetName().c_str());
    }
    else
        sLog->outDebug(LOG_FILTER_MODULES, "CrossFaction: tried to update faction of player %u but he's not online... ", GUID_LOPART(guid));
}
...
void CrossFaction::UpdateAllGroups()
{
    for (UNORDERED_MAP<uint64, uint8>::iterator itr = LeaderRaceMap.begin(); itr != LeaderRaceMap.end(); itr++)
        if (Player* leader = ObjectAccessor::FindPlayer(itr->first))
            ...
            {
                sLog->outDebug(LOG_FILTER_MODULES, "Updating faction for group of leader %s", leader->GetName().c_str());
...
public:
    ...
    void OnAddToBattleground(Player* player, Battleground* bg) override
    {
        if (player && bg)
        {
            sLog->outDebug(LOG_FILTER_MODULES, "adding player %u to bg", player->GetGUID());
    ...
    void OnRemoveFromBattleground(Player* player, Battleground* bg) override
    {
        if (player && bg)
        {
            sLog->outDebug(LOG_FILTER_MODULES, "removing player %u from bg", player->GetGUID());
