...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
	...
	uint8 CFSteam = GetOtherTeamId(player->GetTeam() == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
    ...
    switch (questid)
    {
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            RewardReputationToTeam(teamId, CFSteam, 1, teamId);
...
void BattlegroundAV::EndBattleground(TeamId winnerTeamId)
{
    ...
    for (TeamId iTeamId = TEAM_ALLIANCE; iTeamId <= TEAM_HORDE; iTeamId = TeamId(iTeamId+1))
    {
        ...
        if (rep[iTeamId] != 0)
			RewardReputationToTeam(729, 730, rep[iTeamId], iTeamId == TEAM_ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
        if (kills[iTeamId] != 0)
