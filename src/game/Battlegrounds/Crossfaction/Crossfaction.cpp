		...
		for (Battleground::BattlegroundPlayerMap::const_iterator itr = pBattleGround->GetPlayers().begin(); itr != pBattleGround->GetPlayers().end(); ++itr)
		{
			if (Player* pPlayer = ObjectAccessor::FindPlayer(itr->first))
			{
				if (GetDistance2d(pPlayer->GetPositionX(), pPlayer->GetPositionY()) <= distance)
				{
					...
					if (msgtype != CHAT_MSG_EMOTE)
						language = sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT) ? LANG_UNIVERSAL : (pPlayer->GetBgTeamId() == TEAM_ALLIANCE ? LANG_COMMON : LANG_ORCISH);

					ChatHandler::BuildChatPacket(data, msgtype, language, this, this, message);
...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
{
	...
	// Check if the system is enabled and we have an even number of people queued.
	if (sWorld->getBoolConfig(CROSSFACTION_SYSTEM_BATTLEGROUNDS) && !(queuedPeople % 2))
	{
